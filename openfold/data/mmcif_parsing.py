...
def safe_pickle_dump(file_name, data):
    nowtime = time.time()
    tempfile_name = file_name + "_temp_" + str(nowtime)
    if os.path.exists(tempfile_name):
        os.remove(tempfile_name)
    pickle.dump(data, open(tempfile_name, "wb"))
    os.rename(tempfile_name, file_name)
...
def parse(
    *, file_id: str, mmcif_string: str, catch_all_errors: bool = True
    ...
    errors = {}
    try:
        # Use the cached mmcif parsing result
