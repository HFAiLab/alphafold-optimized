...
class OpenFoldSingleDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if(mapping_path is None):
            mmcifNames = set([os.path.splitext(name)[0] for name in os.listdir(self.data_dir)])
            exsitingAlignments = [name for name in os.listdir(alignment_dir) if name.split("_")[0] in mmcifNames]
            self.mapping = {
                str(i):os.path.splitext(name)[0] 
                for i, name in enumerate(exsitingAlignments)
            }
        else:
...
class OpenFoldDataModule(pl.LightningDataModule):
    ...
    def _gen_dataloader(self, stage, replace_ddp_sampler=False):
        generator = torch.Generator()
