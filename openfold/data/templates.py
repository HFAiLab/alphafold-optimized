...
def _extract_template_features(
    ...
    try:
        ...
        logging.warning("Realign time cost:",round(end-start,3))
        logging.info(
...
def _process_single_hit(
    ...
    # Replace original with ffrecord reading
    cif_path = os.path.join(mmcif_dir, "mmcif_files", hit_pdb_code)
    if hit_pdb_code in mmcif_index:
        idx = [mmcif_index[hit_pdb_code]]
        data = mmcif_reader.read(idx)
        cif_string = pickle.loads(data[0])
    else:
        cif_string = ""
...
class TemplateHitProcessManager(BaseManager):
    pass

def createTemplateHitPorcessManager():
    m = TemplateHitProcessManager()
    m.start()
    return m

class TemplateHitProcessController():
    def __init__(self, max_hits = 4):
        self.resultlist = []
        self.hitcount = 0
    
    def getHitState(self):
        if self.hitcount >= max_hits:
            return False
        return True
    
    def reporthit(self, data):
...
TemplateHitProcessManager.register('TemplateHitProcessController', TemplateHitProcessController)
...
class TemplateHitFeaturizer:
    ...
    def get_templates(
        ...
        mmcif_reader = FileReader(os.path.join(self._mmcif_dir, "mmcifs.ffr"), check_data=True)
        with open(os.path.join(self._mmcif_dir, "mmcifs_index.pk"), "rb") as fp:
            mmcif_index = pickle.load(fp)
        ...
        TIME_LIMIT = config.feat_cut.template_time_limit
        for id_hit, i in enumerate(idx):
            ...
            if tot_time > TIME_LIMIT:
                # logging.warning(f"Template search has exceeded {TIME_LIMIT} seconds in {id_hit} iters")
                break
