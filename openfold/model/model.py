...
class AlphaFold(nn.Module):
    ...
    def iteration(self, feats, m_1_prev, z_prev, x_prev, _recycle=True):
        ...
        if self.config.template.enabled:
            ...
            # Fix the bug of OpenFold.
            # If the number of template_mask is 0, skip template embedding.
            # We zero them this way instead of computing them
            # conditionally to avoid leaving parameters unused, which has annoying
            # implications for DDP training.
            maskState = torch.any(template_mask)
            multiplier = 0
            if maskState:
                multiplier = 1
            template_feats = {
            ...
            # [*, N, N, C_z]
            z = z + template_embeds["template_pair_embedding"] * multiplier
            
            angleEmbedState = maskState and self.config.template.embed_angles
            # [*, S = S_c + S_t, N, C_m]
            if angleEmbedState:
                m = torch.cat(
                ...
                zero_template = torch.cat(
                    [template_embeds["template_angle_embedding"], torch.zeros((1,124,256,256)).to(m.device)],
                    dim = -3
                )
                m += 0 * zero_template
                torsion_angles_mask = feats["template_torsion_angles_mask"]
                zero_anglemask = torch.cat(
                    [torsion_angles_mask[..., 2], torch.zeros((1,124,256)).to(msa_mask.device)],
                    dim = -2
                )
                msa_mask += feats["msa_mask"] * 0 + zero_anglemask * 0
